...
def write_signals_json(signals: Dict[str, Any], output_prefix: str) -> str:
    """
    Writes <prefix>.signals.json
    - If prefix ends with .jsonl, drop it first
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(signals, indent=2, sort_keys=True, default=str), encoding="utf-8")
    return str(path)


def write_signals_md(signals: Dict[str, Any], output_prefix: str) -> str:
    """
    Writes <prefix>.signals.md
    - If prefix ends with .jsonl, drop it first
    Returns path as string.
    """
    out = Path(output_prefix)
