...
- `logs/example-login.jsonl`  
- `logs/example-login.summary.json`  
- `logs/example-login.summary.md`
...
```toml
[flows.example-login-sequence]
description = "Login as admin, then fetch about page."
steps = ["login", "about"]

[flows.example-login-sequence.login]
method = "POST"
url = "https://example.com/login.php"
body_template = "user=admin&pass={password}"
capture_cookies = true

[flows.example-login-sequence.about]
method = "GET"
...
python -m cate.cli http-flow \
  --flow example-login-sequence \
  --output logs/example-flow
```

Produces:

- `logs/example-flow.jsonl`  
- `logs/example-flow.summary.md`
...
|-------|--------|---------------------------------------|--------|-----|--------------:|------:|-------|
| login | POST   | https://example.com/login.php       | 200    | ✅  | 421.0         | 2512  |       |
| about | GET    | https://example.com/about.html      | 200    | ✅  | 98.6          | 19298 |       |
...
```[CATE] Available flows:
  - example-login-sequence: Login as admin, then fetch about page.
  - example-login-sequence-vars: Login as admin, then fetch about page (with variable extraction demo).
```
...
python -m cate.cli http-flow \
  ...
  --flow example-login-sequence-vars \
...
[flows.example-login-sequence-vars.about]
method = "GET"
url = "https://example.com/about.html?m={marker}"
expect_status = 200
body_must_contain = "About"

[flows.example-login-sequence-vars.echo]
method = "GET"
url = "https://example.com/about.html?marker={marker}&step=echo"
expect_status = 200
```
...
python -m cate.cli http-flow \
  --flow example-login-sequence-vars \
  --output logs/example-flow-vars
```
#### Produces console output:
```
[CATE] Loaded flow 'example-login-sequence-vars'
[CATE] Description: Login as admin, then fetch about page (with variable extraction demo).
[CATE] Steps:
  1. login -> POST https://example.com/login.php       (capture_cookies=True, expect_status=None)
  2. about -> GET  https://example.com/about.html?m={marker}
  3. echo  -> GET  https://example.com/about.html?marker={marker}&step=echo

[CATE] Executing flow (v0.3.0 stateful HTTP run) in env=dev (timeout=10.0s, max_rps=2.0)…

[CATE] Flow results:
[OK] login: POST https://example.com/login.php
     → status=200, 355.9 ms, 2512 bytes

[OK] about: GET https://example.com/about.html?m=Login or Sign Up
     → status=200, 92.5 ms, 19298 bytes

[OK] echo: GET https://example.com/about.html?marker=Login or Sign Up&step=echo
     → status=200, 101.7 ms, 19298 bytes

[CATE] Flow logs written to:
  logs/example-flow-vars.jsonl
  logs/example-flow-vars.summary.md

[CATE] Flow completed successfully.
```
#### And the generated Markdown report (example-flow-vars.summary.md) looks like:
...
| Step  | Method | URL                                                                | Status | OK  | Latency (ms) | Bytes | Error |
|-------|--------|--------------------------------------------------------------------|--------|-----|--------------:|-------:|-------|
| login | POST   | https://example.com/login.php                                   | 200    | ✅  | 1008.6       | 2512  |       |
| about | GET    | https://example.com/about.html?m=Login or Sign Up               | 200    | ✅  |   88.6       | 19298 |       |
| echo  | GET    | https://example.com/about.html?marker=Login or Sign Up&step=echo | 200    | ✅  |  101.7       | 19298 |       |
